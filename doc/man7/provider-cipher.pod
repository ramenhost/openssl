...
the non-pipeline variants, but are used when the application is using cipher pipeline.
...
All arrays are of length I<numpipes>. See L<EVP_CipherPipelineInit(3)> for more
information.
