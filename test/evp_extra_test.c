...
static int test_evp_cipher_pipeline(void)
{
    ...
    if (!TEST_false(EVP_CIPHER_can_pipeline(EVP_aes_256_gcm(), 1)))
        goto end;
    if (!TEST_false(EVP_CipherPipelineInit(ctx, pipeline_cipher, key,
                                           EVP_MAX_PIPES + 1, NULL, 1)))
        goto end;

    /* Positive tests */
    for (numpipes = 1; numpipes <= EVP_MAX_PIPES; numpipes++) {
        for (size_t plaintextlen = 1; plaintextlen <= 256; plaintextlen++) {
            ...
            if (!TEST_true(EVP_CIPHER_CTX_reset(ctx))
                ...
                || !TEST_true(EVP_CipherPipelineInit(ctx, pipeline_cipher, key,
                                                     numpipes,
                                                     (const unsigned char **)iv_array,
                                                     1))
                /* reuse plaintext for AAD as it won't affect test */
                || !TEST_true(EVP_CipherPipelineUpdate(ctx, NULL,
                                                       outlen_array,
            ...
            if (!TEST_true(EVP_CipherPipelineFinal(ctx, temp, outlen_array)))
                goto err;
            ...
            for (size_t i = 0; i < numpipes; i++) {
                if (!TEST_true(EVP_EncryptInit_ex(ctx, cipher, NULL, key,
                                                  iv_array[i]))
                    || !TEST_true(EVP_EncryptUpdate(ctx, NULL, &outlen,
                                                    plaintext_array[i],
                                                    plaintextlen))
                ...
                if (!TEST_true(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, taglen, tag)))
                    goto err;
